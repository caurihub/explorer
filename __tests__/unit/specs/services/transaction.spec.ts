...
describe("Services > Transaction", () => {
  beforeAll(() => {
    store.dispatch("network/setServer", "https://node.cauri.cm/api");
  });
  ...
  it("should return latest transactions for an address and page offset", async () => {
    const { data } = await TransactionService.allByAddress("CUexKjGtgsSpVzPLs6jNMM6vJ6znEVTQWK", 1);
    expect(data).toHaveLength(25);
    data.forEach(transaction => {
      transaction.signatures
        ? expect(Object.keys(transaction).sort()).toEqual(expect.arrayContaining(multiSigTransactionPropertyArray))
  ...
  it("should return latest transactions for an address with page offset and given limit", async () => {
    const { data } = await TransactionService.allByAddress("CUexKjGtgsSpVzPLs6jNMM6vJ6znEVTQWK", 2, 40);
  ...
  it("should return all outgoing transactions for an address", async () => {
    const { data } = await TransactionService.sentByAddress("CYCTHSZionfGoQsRnv5gECEuFWcZXS38gs");
    expect(data).toHaveLength(25);
    data.forEach(transaction => {
      transaction.signatures
        ? expect(Object.keys(transaction).sort()).toEqual(expect.arrayContaining(multiSigTransactionPropertyArray))
  ...
  it("should return all outgoing transactions for an address with page offset", async () => {
    const { data } = await TransactionService.sentByAddress("CYCTHSZionfGoQsRnv5gECEuFWcZXS38gs", 3, 40);
    expect(data).toHaveLength(40);
    data.forEach(transaction => {
      transaction.signatures
        ? expect(Object.keys(transaction).sort()).toEqual(expect.arrayContaining(multiSigTransactionPropertyArray))
  ...
  it("should return all incoming transactions for an address", async () => {
    const { data } = await TransactionService.receivedByAddress("CUexKjGtgsSpVzPLs6jNMM6vJ6znEVTQWK");
    expect(data).toHaveLength(25);
    data.forEach(transaction => {
      transaction.signatures
        ? expect(Object.keys(transaction).sort()).toEqual(expect.arrayContaining(multiSigTransactionPropertyArray))
  ...
  it("should return all incoming transactions for an address with page offset", async () => {
    const { data } = await TransactionService.receivedByAddress("CUexKjGtgsSpVzPLs6jNMM6vJ6znEVTQWK", 3, 40);
    expect(data).toHaveLength(40);
    data.forEach(transaction => {
      transaction.signatures
        ? expect(Object.keys(transaction).sort()).toEqual(expect.arrayContaining(multiSigTransactionPropertyArray))
  ...
  it("should fail when searching for outgoing transactions if address does not exist", async () => {
    await expect(TransactionService.sentByAddress("CYCTHSZionfGoQsRnv5gECEuFWcZXS38gz")).rejects.toThrow();
  });

  it("should fail when searching for incoming transactions if address does not exist", async () => {
    await expect(TransactionService.receivedByAddress("CYCTHSZionfGoQsRnv5gECEuFWcZXS38gz")).rejects.toThrow();
  });

  it("should return count of outgoing transactions for an address", async () => {
    const data = await TransactionService.sentByAddressCount("CUexKjGtgsSpVzPLs6jNMM6vJ6znEVTQWK");
    expect(Number(data)).toBeGreaterThan(0);
  });

  it("should return count of incoming transactions for an address", async () => {
    const data = await TransactionService.receivedByAddressCount("CUexKjGtgsSpVzPLs6jNMM6vJ6znEVTQWK");
    expect(Number(data)).toBeGreaterThan(0);
  });
  ...
  it("should return all transactions with a fee exceeding 25 CAU", async () => {
  ...
  it("should return all transactions with an amount between 5000 and 6000 CAU", async () => {
...
describe("Services > Transaction (2.6)", () => {
  beforeAll(() => {
    store.dispatch("network/setServer", "https://dnode.cauri.cm/api/v2");
  });
