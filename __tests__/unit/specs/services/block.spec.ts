...
describe("Services > Block", () => {
  ...
  it("should return the blocks for given generator address", async () => {
    jest.setTimeout(30000);
    const { data } = await BlockService.byAddress("CeaqhUKfBtVqNhtMct3piBiWfdhbRwbg4W");
    expect(data).toHaveLength(25);
    data.forEach((block) => {
      expect(Object.keys(block).sort()).toEqual(blockPropertyArray);
    });
  ...
  it("should return all blocks with a total fee exceeding 25 CAU ", async () => {
  ...
  it("should return all blocks with a total amount between 100 and 500 CAU", async () => {
