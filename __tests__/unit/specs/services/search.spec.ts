...
describe("Services > Search", () => {
  ...
  it("should return an object when searching for existing wallet", async () => {
    const data = await SearchService.walletByAddress("CTsPMTAHNsUwKedzNpjTNRfcj1oRGaX5xC");
  ...
  it("should fail when searching for non-existing wallet", async () => {
    await expect(SearchService.walletByAddress("CTsPMTAHNsUwKedzNpjTNRfcj1oRGaX5xz")).rejects.toThrow();
  });

  it("should return an object when searching for existing username", async () => {
    const data = await SearchService.delegateByQuery("cauripool");
